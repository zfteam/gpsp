    ...
    clear_screen(COLOR_BG);
  {
    while(repeat)
    {
      ...
      switch(gui_action)
      {
        ...
        case CURSOR_SELECT:
          // printf("cursor_select\n");
          // break;
          if(current_column == 1)
          {
            // printf("cursor_select1\n");
            repeat = 0;
            chdir(dir_list[current_dir_selection]);
          }
          else
          {
            // printf("cursor_select21\n");
            if(num_files != 0)
            {
              repeat = 0;
              return_value = 0;
              printf("cursor_select22\n");
              strcpy(result, file_list[current_file_selection]);
              printf("cursor_select23\n");
            }
          }
          break;
